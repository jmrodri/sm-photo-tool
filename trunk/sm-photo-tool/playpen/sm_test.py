...
class Smugmug:
    ...
    print rsp[0].tagName
        ...
        conn.putrequest('PUT', filename)

        conn.putheader("User-Agent", 'sm_json.py/1.0')
        conn.putheader("Connection", 'keep-alive')
        conn.putheader("Transfer-Encoding", 'chunked')
        conn.putheader("Expect", '100-continue')
        conn.putheader("Accept", '*/*')
        conn.putheader("Content-MD5",md5.new(data).hexdigest())
        conn.putheader("X-Smug-SessionID",sessionid)
        conn.putheader("X-Smug-Version",'1.1.1')
        conn.putheader("X-Smug-ResponseType", 'REST')
        conn.putheader("X-Smug-AlbumID", albumid)
        conn.putheader("X-Smug-FileName", filename)
        conn.endheaders()
